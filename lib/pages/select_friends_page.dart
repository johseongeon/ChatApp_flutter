...
class _SelectFriendsPageState extends State<SelectFriendsPage> {
  ...
  Future<void> fetchFriends() async {
    final url = Uri.parse('$serverUrl:8082/getFriends?username=${widget.username}');
  ...
  for (var friend in selectedFriends) {
    final url = Uri.parse('$serverUrl:8082/joinUser?username=$friend&room_id=$roomId');
    futures.add(http.get(url));
  }

  // 현재 사용자도 방에 join
  final currentUserUrl = Uri.parse('$serverUrl:8082/joinUser?username=${widget.username}&room_id=$roomId');
